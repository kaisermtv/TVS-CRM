...
public class Account
{
    ...
    public int setData(int Id, string UserName, string FullName, string Email, string PassWord, int GroupId, int TypeId, int ProvincerId, bool State)
    {
        int tmpValue = 0;
        try
        {
            ...
            sqlQuery += "BEGIN INSERT INTO tblAccount(UserName,FullName,Email,PassWord,GroupId,TypeId,ProvincerId,State) VALUES(@UserName,@FullName,@Email,@PassWord,@GroupId,@TypeId,@ProvincerId,@State) END ";
            sqlQuery += "ELSE BEGIN UPDATE tblAccount SET FullName = @FullName, Email = @Email, GroupId = @GroupId, TypeId = @TypeId, ProvincerId = @ProvincerId, State = @State  WHERE Id = @Id END ";
    ...
    public DataTable getData(string searchKey)
    {
        ...
        if (searchKey.Trim() != "")
        {
            sqlQuery += " AND (UPPER(RTRIM(LTRIM(FullName))) LIKE N'%'+UPPER(RTRIM(LTRIM(@SearchKey)))+'%' OR UPPER(RTRIM(LTRIM(UserName))) LIKE N'%'+UPPER(RTRIM(LTRIM(@SearchKey)))+'%')";
        }
    ...
    #region method getDataByProvincer
    public DataTable getDataByProvincer(string searchKey, string UserName)
    {
        string sqlQuery = "";
        if (searchKey.Trim() != "")
        {
        ...
        try
        {
            ...
            Cmd.Parameters.Add("SearchKey", SqlDbType.NVarChar).Value = searchKey;
            Cmd.Parameters.Add("UserName", SqlDbType.NVarChar).Value = UserName;
            Cmd.CommandText = "SELECT 0 AS TT, *, (SELECT Name FROM tblAccountGroup WHERE Id = GroupId) AS GroupName, REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(CAST(TypeId AS varchar),'1',N'Quản lý công ty'),'0',N'Quản lý hệ thống'),'2',N'<a href = \"AccountProvincer.aspx?acc='+UserName+N'\"><img src = \"../images/69.png\"></a>&nbsp;&nbsp;Quản lý chi nhánh'),'3',N'<a href = \"AccountDistrict.aspx?acc='+UserName+N'\"><img src = \"../images/35.png\"></a>&nbsp;&nbsp;Quản lý nhóm'),'4',N'Người dùng (AM)') AS TypeName, REPLACE(REPLACE(CAST(State AS varchar),'1',N'Kích hoạt'),'0',N'Đóng') AS StateName FROM tblAccount WHERE ProvincerId = (SELECT TOP 1 ProvincerId FROM tblAccount WHERE UserName = @UserName) " + sqlQuery;
            SqlDataAdapter da = new SqlDataAdapter();
            da.SelectCommand = Cmd;
            DataSet ds = new DataSet();
            da.Fill(ds);
    ...
    public DataTable getDataToCombobox(DateTime objDate1, DateTime objDate2, int ProvincerId)
    {
        string sqlQueryMore = "";
        if (ProvincerId > 0)
        {
            sqlQueryMore = " AND tblAccount.ProvincerId = @ProvincerId";
        }
        DataTable objTable = new DataTable();
        try
        {
            ...
            sqlCon.Open();
            SqlCommand Cmd = sqlCon.CreateCommand();
            Cmd.CommandText = "SELECT DISTINCT tblAccount.Id, FullName FROM tblAccount,tblCustomerTask WHERE tblAccount.Id = tblCustomerTask.ContactId AND DayBegin BETWEEN @objDate1 AND @objDate2 " + sqlQueryMore;
            Cmd.Parameters.Add("ProvincerId", SqlDbType.Int).Value = ProvincerId;
    ...
    public bool checkForLogin(string UserName, string Password, ref string FullName, ref string TypeId)
    {
        bool tmpValue = false;
        try
        {
    ...
    #region method delLevelDistrictId
    public void delLevelDistrictId(string userName, int LevelDistrictId)
    {
        try
        {
            string sqlQuery = "";
            sqlQuery = "DELETE tblAccountLocation WHERE UserName = @UserName AND LevelDistrictId = @LevelDistrictId ";
            SqlConnection sqlCon = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["TVSConn"].ConnectionString);
            sqlCon.Open();
            SqlCommand Cmd = sqlCon.CreateCommand();
            Cmd.CommandText = sqlQuery;
            Cmd.Parameters.Add("userName", SqlDbType.NVarChar).Value = userName;
            Cmd.Parameters.Add("LevelDistrictId", SqlDbType.Int).Value = LevelDistrictId;
            Cmd.ExecuteNonQuery();
            sqlCon.Close();
            sqlCon.Dispose();
        }
