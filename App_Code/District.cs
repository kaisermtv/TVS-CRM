...
public class District
{
    ...
    #region method getDataManagement
    public DataTable getDataManagement(int ProvincerId, string searchKey, string UserName)
    {
        string sqlQuery = "";
        if (searchKey.Trim() != "")
        {
        ...
        try
        {
            ...
            SqlCommand Cmd = sqlCon.CreateCommand();
            Cmd.Parameters.Add("SearchKey", SqlDbType.NVarChar).Value = searchKey;
            Cmd.CommandText = "SELECT 0 AS TT, *, REPLACE(REPLACE(CAST(State AS varchar),'1',N'Kích hoạt'),'0',N'Đóng') AS StateName, ISNULL((SELECT COUNT(*) FROM tblWard WHERE DistrictId = tblDistrict.Id),0) AS CountItem FROM tblDistrict WHERE ProvincerId = @ProvincerId AND Id IN (SELECT LevelDistrictId FROM tblAccountLocation WHERE UserName = @UserName AND LevelProvincerId = 0 AND LevelWardId = 0) " + sqlQuery;
            Cmd.Parameters.Add("ProvincerId", SqlDbType.Int).Value = ProvincerId;
            Cmd.Parameters.Add("UserName", SqlDbType.NVarChar).Value = UserName;
            SqlDataAdapter da = new SqlDataAdapter();
            da.SelectCommand = Cmd;
    ...
    public DataTable getDataCategoryToCombobox(string ProvincerId)
    {
        ...
        try
        {
            ...
            sqlCon.Dispose();
            objTable = ds.Tables[0];
            objTable.Rows.Add(0, "Quận, huyện");
        }
        catch
        {
