...
public class Activity
{
    ...
    public DataTable getData(string UserName, int TypeId)
    {
        ...
        try
        {
            ...
            if (TypeId == 1)
            {
                Cmd.CommandText = "SELECT * FROM tblActivity WHERE Datediff(day,DayCreate,getdate()) <= 30 ORDER BY DayCreate DESC";
            }
            else
            {
                Cmd.CommandText = "SELECT * FROM tblActivity WHERE Datediff(day,DayCreate,getdate()) <= 70 AND UserName = @UserName ORDER BY DayCreate DESC";
            }
            Cmd.Parameters.Add("UserName", SqlDbType.NVarChar).Value = UserName;
